...
order = request("/api/orders", "POST", {"sku": "KEY-GTA5", "order_id": order_id})
...
assert request(f"/api/orders/{order_id}")["status"] == "reserved", "amount mismatch incorrectly confirmed payment"

def send_webhook(index):
    return request("/api/webhooks/payment", "POST", {"event_id": f"evt_{order_id}_{index}", "order_id": order_id, "status": "paid", "amount": 1990})
...
duplicate = request("/api/webhooks/payment", "POST", {"event_id": f"evt_{order_id}_0", "order_id": order_id, "status": "paid", "amount": 1990})
